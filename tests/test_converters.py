...
class test(unittest.TestCase):
    def check_remove(self, type: str, output_folder="", output_filename="", input_filepath=""):
        ...
        if (type == "excel"):
            ...
            self.assertEqual(os.path.exists(os.path.join(
                output_folder, output_filename)), True)
            actual_df = pd.read_csv(input_filepath)
            converted_df = pd.read_excel(
    ...
    def test_png_to_jpg(self):
        ...
        convert__image_png_to_jpg(
            input_filepath=input_filepath_path, output_filename=output_filename)
        self.check_remove(
            "jpg_image", output_filename=output_filename, input_filepath=input_filepath_str)
        convert__image_png_to_jpg(
            input_filepath=input_filepath_path, output_folder=output_folder_path)
        self.check_remove("jpg_image", output_folder=output_folder_path,
                          input_filepath=input_filepath_path)
        convert__image_png_to_jpg(input_filepath=input_filepath_path,
                                  output_folder=output_folder_path, output_filename=output_filename)
    ...
    def test_base_to_img(self):
        ...
        self.check_remove("base_image", output_folder=output_folder,
                          input_filepath=input_img_base64)
        get_image_from_base64(
            input_img_base64, output_filename=output_filename)
        self.check_remove(
            "base_image", output_filename=output_filename, input_filepath=input_img_base64)
    ...
    def test_colored_html(self):
        ...
        self.check_remove(
            "colored_html", output_folder=output_folder_str, input_filepath=input_file_str)
        excel_to_colored_html(input_file_str, output_filename=output_filename)
        self.check_remove("colored_html", default_out_path,
                          output_filename, input_file_str)
        excel_to_colored_html(
            input_file_str, output_folder=output_folder_str, output_filename=output_filename)
        self.check_remove("colored_html", output_folder_str,
                          output_filename, input_file_str)
        ...
        self.check_remove("colored_html", input_filepath=input_file_str)
        excel_to_colored_html(
            input_file_path, output_folder=output_folder_path)
        self.check_remove(
            "colored_html", output_folder=output_folder_str, input_filepath=input_file_str)
        ...
        self.check_remove("colored_html", default_out_path,
                          output_filename, input_file_str)
        excel_to_colored_html(
            input_file_path, output_folder=output_folder_path, output_filename=output_filename)
        self.check_remove("colored_html", output_folder_str,
                          output_filename, input_file_str)
